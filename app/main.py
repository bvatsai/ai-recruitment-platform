...
from app.api.routers.resume_router import router as resume_router #jumps to the file app/api/routers/resume_router.py and imports the router instance defined in that file. This router contains all the resume-related endpoints, such as uploading a resume, retrieving resumes, etc.
...
app.include_router(resume_router)       #Includes Router for resume endpoints which is file app/api/routers/resume_router.py. This router handles all the resume-related endpoints, such as uploading a resume, retrieving resumes, etc.
